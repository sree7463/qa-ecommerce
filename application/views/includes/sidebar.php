	...
	<div id="sidebar" class="span3">
	  ...
	  <ul id="sideManu" class="nav nav-tabs nav-stacked">
		  <?php foreach ($sorted_categories as $category): ?>
			  <li <?php if (property_exists($category, 'children')) echo 'class="subMenu open"'; ?>>
			    <a <?php if (!property_exists($category, 'children')) echo 'href="'. base_url() . seo_friendly_url(strtolower($category->name)) . '-1' .$category->id.'"' ?>><?php echo strtoupper($category->name); ?> </a>
			    <?php if (property_exists($category, 'children')): ?>
			      <ul>
			        <?php foreach ($category->children as $child): ?>
			          <li>
			          	<a href="<?Php echo base_url() . seo_friendly_url(strtolower($child->name)) . '-1' . $child->id; ?>">
...
<!-- 		<ul id="sideManu" class="nav nav-tabs nav-stacked">
		...
		<p>
		  ...
		  ?>
		  <form action="<?php echo base_url($url); ?>" method="POST" id="price_range_form">
		  	<input type="text" name="price_range" id="amount" class="price-filter-inp" readonly>
		  </form>
